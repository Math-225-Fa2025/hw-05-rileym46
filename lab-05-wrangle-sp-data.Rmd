...
1.  Filter the Denny's dataframe for Alaska (AK) and save the result as `dn_ak`. How many Denny's locations are there in Alaska? `3`
...
dn_ak
```
2.  Filter the La Quinta dataframe for Alaska (AK) and save the result as `lq_ak`. How many La Quinta locations are there in Alaska? `2`
...
`There are 6 distances we need to calculate.`
...
4.  How many observations are in the joined `dn_lq_ak` data frame? What are the names of the variables in this data frame. `There are 6 observavations. The variables are Address.x, city.x, state, zip.x, longitude.x, latitude.x, address.y, city.y, zip.y, latitude.y, and longitude.y`
...
5.  What function from the tidyverse do we use the add a new variable to a data frame while keeping the existing variables? `Mutate.`
...
8.  Describe the distribution of the distances Denny's and the nearest La Quinta locations in Alaska. Also include an appripriate visualization and relevant summary statistics. `Some Denny's are relatively close to a La Quinta, but the others are further. Each Denny's has a La Quinta within 6 km.`
...
nrow(lq_nc)
```
```{r full-join-nc, eval = TRUE}
dn_lq_nc <- full_join(dn_nc, lq_nc, by = "state")
dn_lq_nc
```

```{r distance-nc, eval = TRUE}
dn_lq_nc %>%
  mutate(distance = haversine(longitude.x,latitude.x,longitude.y,latitude.y))
```
```{r min-dist-nc, eval = TRUE}
dn_lq_nc %>%
  mutate(distance = haversine(longitude.x,latitude.x,longitude.y,latitude.y)) %>%
  group_by(address.x) %>%
  summarise(closest = min(distance)) %>%
...
nrow(lq_tx)
```
```{r full-join-tx, eval = TRUE}
dn_lq_tx <- full_join(dn_tx, lq_tx, by = "state")
dn_lq_tx
```

```{r distance-tx, eval = TRUE}
dn_lq_tx %>%
  mutate(distance = haversine(longitude.x,latitude.x,longitude.y,latitude.y))
```
```{r min-dist-tx, eval = TRUE}
dn_lq_tx %>%
  mutate(distance = haversine(longitude.x,latitude.x,longitude.y,latitude.y)) %>%
  group_by(address.x) %>%
  summarise(closest = min(distance)) %>%
...
```
11. Repeat the same analysisfor a state of your choosing, different than the ones we covered so far.
...
nrow(lq_ca)
```
```{r full-join-ca, eval = TRUE}
dn_lq_ca <- full_join(dn_ca, lq_ca, by = "state")
dn_lq_ca
```

```{r distance-ca, eval = TRUE}
dn_lq_ca %>%
  mutate(distance = haversine(longitude.x,latitude.x,longitude.y,latitude.y))
```
```{r min-dist-ca, eval = TRUE}
dn_lq_ca %>%
  mutate(distance = haversine(longitude.x,latitude.x,longitude.y,latitude.y)) %>%
  group_by(address.x) %>%
  summarise(closest = min(distance)) %>%
...
`It is difficult to tell because there are so many different locations. Alaska consistently has Denny's close to LaQuintas, but there are also fewer locations. Larger states like Texas and California have shorter distances, but also further maximum distances. I would say that Mitch Hedberg's joke is more accurate in Texas, as it has a smaller maximum distance than any in California.`
